...
namespace CatalogService.Api.Infrastructure.Context
{
    public class CatalogContextSeed
    {
        ...
        {
            var table = context.Set<CatalogBrand>();
            var tableType = context.Set<CatalogType>();
            var tableItem = context.Set<CatalogItem>();


            if (!table.Any())
            {
                await table.AddRangeAsync(GetCatalogBrandsFromFile(setupDirPath));
            ...
            if (!tableType.Any())
            {
                await tableType.AddRangeAsync(GetCatalogTypesFromFile(setupDirPath));
            ...
            if (!tableItem.Any())
            {
                await tableItem.AddRangeAsync(GetCatalogItemsFromFile(setupDirPath, context));
        ...
        private IEnumerable<CatalogItem> GetCatalogItemsFromFile(string contentPath, CatalogContext context)
        {
            ...
            var catalogTypeIdLookup = tableType.ToDictionary(ct => ct.Type, ct => ct.Id);
            var catalogBrandIdLookup = table.ToDictionary(ct => ct.Brand, ct => ct.Id);
